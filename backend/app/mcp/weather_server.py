...
def _get_city_adcode(city: str) -> str | None:
    """根据城市名获取行政区划代码"""
    try:
        from app.mcp.amap_server import geocode_address
...
def get_weather_forecast(city: str) -> dict[str, Any]:
    ...
    Returns:
        包含 city, days（每日天气列表）等字段的字典
    """
    cache_key = f"mcp:weather:forecast:{_normalize_cache_text(city)}"
    cached_value = get_cached_json(cache_key)
    if cached_value is not None:
        logger.info("MCP weather cache hit: city=%s", city)
        return cached_value
...
def get_current_weather(city: str) -> dict[str, Any]:
    ...
    Returns:
        包含实时天气信息的字典
    """
    cache_key = f"mcp:weather:current:{_normalize_cache_text(city)}"
    cached_value = get_cached_json(cache_key)
    if cached_value is not None:
        logger.info("MCP current weather cache hit: city=%s", city)
        return cached_value
...
@mcp.tool()
def get_weather_alert(city: str) -> dict[str, Any]:
    """获取指定城市的天气预警信息
    ...
    Returns:
        包含天气预警信息的字典
    """
    cache_key = f"mcp:weather:alert:{_normalize_cache_text(city)}"
    cached_value = get_cached_json(cache_key)
    if cached_value is not None:
        logger.info("MCP weather alert cache hit: city=%s", city)
        return cached_value
...
@mcp.tool()
def get_weather_suggestion(city: str) -> dict[str, Any]:
    """获取指定城市的天气生活指数
    ...
    Returns:
        包含穿衣、紫外线等生活指数的字典
    """
    cache_key = f"mcp:weather:suggestion:{_normalize_cache_text(city)}"
    cached_value = get_cached_json(cache_key)
    if cached_value is not None:
        logger.info("MCP weather suggestion cache hit: city=%s", city)
        return cached_value
...
def _generate_suggestions(weather_data: dict) -> dict[str, str]:
    """根据天气数据生成生活建议"""
    suggestions = {}
